...
void DrawSpan(int x1, int x2, int height) {
  ...
  //
  // Beware - This loop is SLOW, and WILL cause framedrops on slower machines
  // and/or on higher resolutions. I can't stress it enough: this is one of the
  // worst loops in history! No amount of optimisation will change that! This
  // loop SUCKS!
  //
  // Someday flats rendering will be re-written, but until then, YOU HAVE BEEN
  // WARNED.
  //
  while (count--) {
    //
    ...
    if (spot) {
      texture =
          ((xfrac >> FIXED2TEXSHIFT) & TEXTUREMASK) +
