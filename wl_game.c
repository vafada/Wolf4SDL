...
void SetupGameLevel(void) {
  ...
  if (!loadedgame) {
    gamestate.TimeCount = gamestate.secrettotal = gamestate.killtotal =
        gamestate.treasuretotal = gamestate.secretcount = gamestate.killcount =
            gamestate.treasurecount = gamestate.attackframe =
                gamestate.attackcount = gamestate.weaponframe = pwallstate =
                    pwallpos = facetimes = 0;
    LastAttacker = NULL;
  }
...
#define MXX MAPSIZE - 1
  ...
  // Read feature flags data from map corners and overwrite corners with
  // adjacent tiles
...
void DrawPlayBorder(void) {
  ...
  if (bordercol != VIEWCOLOR) {
    ...
    VWB_BarScaledCoord(screenWidth - statusborderw - px * 8,
                       screenHeight - px * STATUSLINES, statusborderw + px * 8,
...
void ShowActStatus() {
  ...
  VL_MemToScreenScaledCoord2(source, width, 9, 4, destx, desty, width - 18,
                             height - 7);
...
void Died(void) {
  ...
  FizzleFade(screenBuffer, viewscreenx, viewscreeny, viewwidth, viewheight, 70,
             false);
...
void GameLoop(void) {
  ...
  do {
    ...
    //        DrawLevel ();                     // ADDEDFIX 5 - moved up  Chris
    //        Chokan
