...
boolean IN_JoyPresent(void) { return Joystick != NULL; }
...
=============================================================================
=
= Input processing is not done via interrupts anymore! Instead you have to call
IN_ProcessEvents = in order to process any events like key presses or mouse
movements. Alternatively you can call = IN_WaitAndProcessEvents, which waits for
an event before processing if none were available.
=
= If you have a loop with a blinking cursor where you are waiting for some key
to be pressed, = this loop MUST contain an
IN_ProcessEvents/IN_WaitAndProcessEvents call, otherwise the program = will
never notice any keypress.
=
= Don't write something like, for example, "while (!keyboard[sc_X])
IN_ProcessEvents();" to = wait for the player to press the X key, as this would
result in 100% CPU usage. Either = use IN_WaitAndProcessEvents, or add
SDL_Delay(5) to the loop, which waits for 5 ms before = it continues.
=
===========================================
*/
...
void IN_ClearTextInput(void) { memset(textinput, 0, sizeof(textinput)); }
