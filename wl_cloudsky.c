...
= the skytable. The according colormapentry_t array defines how these colors
should = be mapped to the game palette. The first number in each entry defines
...
//
// The sky definitions which can be selected as defined by GetCloudSkyDefID() in
// wl_def.h You can use <TAB>+Z in debug mode to find out suitable values for
// seed and colorMapIndex Each entry consists of seed, speed, angle and
// colorMapIndex
//
cloudsky_t cloudSkys[] = {
    {626, 800, 20, 0},   {1234, 650, 60, 1},   {0, 700, 120, 0},
    {0, 0, 0, 0},        {11243, 750, 310, 0}, {32141, 750, 87, 0},
    {12124, 750, 64, 0}, {55543, 500, 240, 0}, {65535, 200, 54, 1},
    {4, 1200, 290, 0},
};
...
void SplitS(unsigned size, unsigned x1, unsigned y1, unsigned x2, unsigned y2) {
  ...
  if (!skyc[((x1 + size / 2) * 256 + y1)]) {
    skyc[((x1 + size / 2) * 256 + y1)] =
        (byte)(((int)skyc[(x1 * 256 + y1)] +
                (int)skyc[((x2 & 0xff) * 256 + y1)]) /
               2) +
        rand() % (size * 2) - size;
    if (!skyc[((x1 + size / 2) * 256 + y1)])
  ...
  skyc[((x1 + size / 2) * 256 + y1 + size / 2)] =
      (byte)(((int)skyc[(x1 * 256 + y1)] + (int)skyc[((x2 & 0xff) * 256 + y1)] +
              (int)skyc[(x1 * 256 + (y2 & 0xff))] +
              (int)skyc[((x2 & 0xff) * 256 + (y2 & 0xff))]) /
             4) +
      rand() % (size * 2) - size;
...
void InitSky(void) {
  ...
  if (curSky->colorMapIndex >= numColorMaps)
    Quit("Illegal colorMapIndex for cloud sky def %u: %u", cloudskyid,
         curSky->colorMapIndex);
  ...
  for (calcedCols = 0; calcedCols < curMap->numColors; curEntry++) {
    if (curEntry->startAndDir < 0) {
      for (i = 0, index = -curEntry->startAndDir; i < curEntry->length;
           i++, index--) {
        if (index < 0)
  ...
  for (j = 0; j < 256; j++) {
    for (i = 0; i < 256; i++)
      skyc[(j << 8) + i] =
          colormap[(skyc[(j << 8) + i] * curMap->numColors) >> 8];
  }
}
