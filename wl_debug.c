    ...
    if (addr) {
      if (i < PMSpriteStart) {
        ...
        for (j = 0; j < pageLengths[i]; j += 32) {
          ...
          if (v2 < 0)
            VWB_Vlin(WindowY + WindowH - 32 + v2, WindowY + WindowH - 32,
                     WindowX + 8 + (j / 32), BLACK);
          else
            VWB_Vlin(WindowY + WindowH - 32, WindowY + WindowH - 32 + v2,
                     WindowX + 8 + (j / 32), BLACK);
        }
      }
    }
...
int DebugKeys(void) {
  ...
  if (Keyboard[sc_F]) // F = facing spot
  {
    ...
    US_Print(str);
    snprintf(str, sizeof(str), " 2: %u",
             MAPSPOT(player->tilex, player->tiley, 1));
    US_Print(str);
    snprintf(str, sizeof(str), " 3: %u",
             !ISPOINTER(spot) ? spotvis[player->tilex][player->tiley]
                              : spot->flags);
    US_Print(str);
    ...
    for (end = ReadShort(linecmds) >> 1; end; end = ReadShort(linecmds) >> 1) {
      ...
      for (src = &linesrc[top + start]; start != end; start++, src++) {
        if (!(x % tilewallratio) && !(start % tilewallratio))
          *(vbuf + ylookup[sy + (start / tilewallratio)] +
            (sx + (x / tilewallratio))) = *src;
      }
      ...
      if (tile) {
        ...
        if (tile < BIT_DOOR && tile != BIT_WALL) {
          ...
          if (DebugOk && ISPOINTER(tile)) {
            ...
            if (spotvis[(byte)(obj->x >> TILESHIFT)]
                       [(byte)(obj->y >> TILESHIFT)]) {
