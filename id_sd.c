...
//                      DigiMode - What device is used for digitized sound
//                      effects
...
//
///////////////////////////////////////////////////////////////////////////
static void SDL_PCStopSound(void) { pcSound = 0; }
...
//
///////////////////////////////////////////////////////////////////////////
static void SDL_ShutPC(void) { pcSound = 0; }
...
Sint16 GetSample(float csample, byte *samples, int size) {
  ...
  float val =
      s0 * sf * (sf - 1) / 2 - s1 * (sf * sf - 1) + s2 * (sf + 1) * sf / 2;
...
void SD_PrepareSound(int which) {
  ...
  int destsamples =
      (int)((float)size * (float)param_samplerate / (float)ORIGSAMPLERATE);
  ...
  Sint16 *newsamples =
      (Sint16 *)(void *)(wavebuffer + sizeof(headchunk) + sizeof(wavechunk));
  float cursample = 0.F;
  ...
  SDL_RWops *temp = SDL_RWFromMem(
      wavebuffer, sizeof(headchunk) + sizeof(wavechunk) + destsamples * 2);
...
void SD_ChannelFinished(int channel) { channelSoundPos[channel].valid = 0; }
...
void SD_Startup(void) {
  ...
  if (Mix_OpenAudioDevice(param_samplerate, AUDIO_S16, 2, chunksize, NULL,
  ...
  //    YM3812Write(0,8,0); // Set CSM=0 & SEL=0		 // already set
  //    in for statement
...
//
///////////////////////////////////////////////////////////////////////////
void SD_MusicOn(void) { sqActive = true; }
