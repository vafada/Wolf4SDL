...
int32_t CA_RLEWCompress(word *source, int32_t length, word *dest,
                        word rlewtag) {
  word value, count;
  unsigned i;
  word *start, *end;
...
void CA_CacheAdlibSoundChunk(int chunk) {
  ...
  fread(sound->data, size - ORIG_ADLIBSOUND_SIZE + 1, 1,
        audiofile); // + 1 because of byte data[1]
...
void CA_CannotOpen(const char *string) { Quit("Can't open %s!", string); }
