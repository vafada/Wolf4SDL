...
boolean PreloadUpdate(unsigned current, unsigned total) {
  ...
  if (w) {
    VWB_BarScaledCoord(WindowX + scaleFactor * 5,
                       ...
                       0x37); // SECONDCOLOR);
    VWB_BarScaledCoord(WindowX + scaleFactor * 5,
                       WindowY + WindowH - scaleFactor * 3, w - scaleFactor * 1,
                       scaleFactor * 1, 0x32);
  }
  VW_UpdateScreen();
...
void PreloadGraphics(void) {
  ...
  VWB_BarScaledCoord(0, 0, screenWidth,
                     screenHeight - scaleFactor * (STATUSLINES - 1), bordercol);
  VWB_DrawPicScaledCoord(((screenWidth - scaleFactor * 224) / 16) * 8,
                         (screenHeight - scaleFactor * (STATUSLINES + 48)) / 2,
                         GETPSYCHEDPIC);
...
#define PRINTCOLOR HIGHLIGHT
  ...
  while (attempt < 3) {
    ...
    case checkmanual: {
      ...
      US_CPrint(STR_MAN1);
      US_CPrint(STR_MAN2);
      snprintf(message, sizeof(message), STR_MAN3 " \"%s\" " STR_MAN4,
               WordStr[whichword]);
      US_CPrint(message);
      VW_UpdateScreen();
      VW_FadeIn();
