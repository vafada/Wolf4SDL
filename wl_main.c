...
void WriteConfig(void) {
  ...
  if (file) {
    ...
    fwrite(&dummyJoystickProgressive, sizeof(dummyJoystickProgressive), 1,
           file);
    int dummyJoystickPort = 0;
...
boolean LoadTheGame(FILE *file, int x, int y) {
  ...
  while (1) {
    ...
    //
    memcpy(newobj, &nullobj,
           sizeof(nullobj) - (sizeof(nullobj.next) + sizeof(nullobj.prev)));
  }
...
void CheckParameters(int argc, char *argv[]) {
  const char *header = {"Wolf4SDL v2.0\n"
  ...
  for (i = 1; i < argc; i++) {
    ...
    else snprintf(error, sizeof(error), "Invalid parameter(s)!");
  }
