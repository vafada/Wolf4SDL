...
void HitHorizDoor(void) {
  ...
  texture = ((xintercept - doorobjlist[doornum].position) >> FIXED2TEXSHIFT) &
            TEXTUREMASK;
...
void HitVertDoor(void) {
  ...
  doornum = tilehit & ~BIT_DOOR;
  texture = ((yintercept - doorobjlist[doornum].position) >> FIXED2TEXSHIFT) &
            TEXTUREMASK;
...
byte vgaCeiling[] = {
#ifndef SPEAR
    0x1d, 0x1d, 0x1d, 0x1d, 0x1d, 0x1d, 0x1d, 0x1d, 0x1d, 0xbf, 0x4e,
...
#ifdef FIXCALCROTATE
  viewangle =
      (int)(player->angle + (centerx - ob->viewx) / (8 * viewwidth / 320.0));
#else
...
void DrawScaleds(void) {
  ...
  for (obj = player->next; obj; obj = obj->next) {
    ...
    if (*visspot || (*(visspot - 1)) || (*(visspot + 1)) ||
        (*(visspot - (MAPSIZE + 1))) || (*(visspot - (MAPSIZE))) ||
        (*(visspot - (MAPSIZE - 1))) || (*(visspot + (MAPSIZE + 1))) ||
        (*(visspot + (MAPSIZE))) || (*(visspot + (MAPSIZE - 1)))) {
      obj->active = ac_yes;
      TransformActor(obj);
      if (!obj->viewheight)
      ...
      if ((pwalldir == di_east && xtilestep == 1) ||
        ...
        if (yinttemp >> TILESHIFT == focalty) {
          if (pwalldir == di_east)
            ...
            xintercept =
                ((focaltx << TILESHIFT) - TILEGLOBAL) + ((64 - pwallpos) << 10);
        ...
        if (xinttemp >> TILESHIFT == focaltx) {
          if (pwalldir == di_south)
            ...
            yintercept =
                ((focalty << TILESHIFT) - TILEGLOBAL) + ((64 - pwallpos) << 10);
    ...
    vertentry:
      ...
      if (tilehit) {
        if (tilehit & BIT_DOOR) {
          ...
          // intersected at, and check to see if the door is open past that
          // point
          //
          ...
          yinttemp = yintercept +
                     (ystep >> 1); // add halfstep to current intercept position

          //
          // midpoint is outside tile, so it hit the side of the wall before a
          // door
          //
          ...
          if (door->action != dr_closed) {
            //
            // the trace hit the door plane at pixel position yintercept, see if
            // the door is closed that much
            //
          ...
          if (pwalldir == di_west || pwalldir == di_east) {
            ...
            if ((pwalldir == di_east && xtile == pwallx &&
              ...
              yintercept = yinttemp;
              xintercept = (((fixed)xtile << TILESHIFT) + TILEGLOBAL) -
                           (pwallposinv << 10);
              yinttile = yintercept >> TILESHIFT;
              tilehit = pwalltile;
            ...
            if ((pwalldir == di_south &&
              ...
              if (xtile == pwallx && yinttile == pwally) {
                ...
                if (pwalldir == di_south)
                  ...
                  yintercept = ((yinttile << TILESHIFT) - TILEGLOBAL) +
                               (pwallposi << 10);
              ...
              if (xtile == pwallx && yinttile == pwally) {
                ...
                if (pwalldir == di_south)
                  yintercept =
                      (yinttile << TILESHIFT) - ((64 - pwallpos) << 10);
                else
                  yintercept =
                      (yinttile << TILESHIFT) + ((64 - pwallpos) << 10);
    ...
    horizentry:
      ...
      if (tilehit) {
        if (tilehit & BIT_DOOR) {
          ...
          xinttemp =
              xintercept +
              (xstep >> 1); // add half step to current intercept position
          ...
          if (door->action != dr_closed) {
            //
            // the trace hit the door plane at pixel position xintercept, see if
            // the door is closed that much
            //
          ...
          if (pwalldir == di_north || pwalldir == di_south) {
            ...
            if ((pwalldir == di_south && xinttile == pwallx &&
              ...
              xintercept = xinttemp;
              yintercept = (((fixed)ytile << TILESHIFT) + TILEGLOBAL) -
                           (pwallposinv << 10);
              xinttile = xintercept >> TILESHIFT;
              tilehit = pwalltile;
            ...
            if ((pwalldir == di_east && (word)xintercept < (pwallposi << 10)) ||
              ...
              if (xinttile == pwallx && ytile == pwally) {
                ...
                if (pwalldir == di_east)
                  ...
                  xintercept = ((xinttile << TILESHIFT) - TILEGLOBAL) +
                               (pwallposi << 10);
              ...
              if (xinttile == pwallx && ytile == pwally) {
                ...
                if (pwalldir == di_east)
                  xintercept =
                      (xinttile << TILESHIFT) - ((64 - pwallpos) << 10);
                else
                  xintercept =
                      (xinttile << TILESHIFT) + ((64 - pwallpos) << 10);
