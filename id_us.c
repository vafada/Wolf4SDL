...
//		to whatever is pointed at by def. Input is restricted to
//maxchars 		chars or maxwidth pixels wide. If the user hits escape (and escok is
//		true), nothing is copied into buf, and false is returned. If the
//		user hits return, the current string is copied into buf, and
//true is 		returned
//
///////////////////////////////////////////////////////////////////////////
boolean US_LineInput(int x, int y, char *buf, const char *def, boolean escok,
  ...
  *olds = '\0';
  ...
  while (!done) {
    ...
    // After each direction change accept the next change after 250 ms and then
    // everz 125 ms
    if (ci.dir != lastdir || (curtime - lastdirtime > TickBase / 4 &&
    ...
    if (checkkey) {
      ...
      for (text = textinput; *text; text++) {
        ...
        if (isprint(*text) && (len < MaxString - 1) &&
